...
export const useDynamicRouter = () => {
  ...
  const pathPrefix = useMemo(() => {
    ...
    // Method 2: From router.asPath with flexible regex - decode first to handle special characters
    ...
    // Method 3: Check window.location as fallback - also decode
    if (typeof window !== 'undefined' && router.pathname.includes('[countrycode-hreflng]')) {
      const windowMatch = decodeURIComponent(window.location.pathname).match(/^\/([a-zA-Z]{2}-[a-zA-Z]{2})/);
      if (windowMatch) {

...
export const useDynamicRouter = () => {
  ...
  // Enhanced push method that adds pathPrefix
  const pushDynamic = (path, options) => {
    return router.push(buildPath(path), undefined, options);
  };
  
  // Enhanced replace method that adds pathPrefix
  const replaceDynamic = (path, options) => {
    return router.replace(buildPath(path), undefined, options);
  };

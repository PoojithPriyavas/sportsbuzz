...
export default function BlogsPage({ blogs = [] }) {
  ...
  const filteredBlogs = selectedCategoryId
    ? blogs.filter((blog) =>
        blog.category && blog.category.includes(selectedCategoryId)
      )
    : blogs;
  ...
  const hasActiveFilters = selectedCategoryId || filterValue !== 'all' || searchTerm !== '';
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [selectedCategoryId, filterValue, searchTerm]);

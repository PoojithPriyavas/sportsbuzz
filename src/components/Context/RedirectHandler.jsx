...
const RedirectHandler = () => {
  ...
  useEffect(() => {
    if (!router.isReady) {
      console.log(' redirect handler-Router not ready, skipping redirect');
      return;
    }

    const currentPath = router.asPath;
    console.log(currentPath, " redirect handler - current path")

    // Check if already has locale format (xx-xx)
    const firstSegment = currentPath.split('/')[1];
    if (firstSegment && /^[a-z]{2}-[a-z]{2}$/i.test(firstSegment)) {
      console.log(' redirect handler-Already localized, skipping redirect');
      return;
    }
    ...
    const redirect = (lang, country) => {
      ...
      console.log(`redirect handler-Redirecting to: ${newPath}`);
